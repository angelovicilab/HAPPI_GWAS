...
farming_with_GAPIT <- function(dat, 
    ...
    colnames(combined_gwas_result) = str_trim(colnames(combined_gwas_result))
    ...
    temp_frequent_snps = combined_gwas_result %>%
        ...
        arrange(desc(Count)) %>%
        as.data.frame(stringsAsFactors = FALSE)
    combined_gwas_result_with_most_occur = combined_gwas_result_with_most_occur %>% left_join(temp_frequent_snps, by = "SNP") %>% as.data.frame(stringsAsFactors = FALSE)
    ...
    tryCatch({
        if(!all(is.na(combined_gwas_result[,2]))){
            ...
            p = ggplot(data = combined_gwas_result_heatmap, aes(x = Position, y = Trait)) +
                labs(y = "Trait", x = "Chromosome Position") +
                facet_grid(. ~ Chromosome, switch = "x", drop = FALSE) +
                geom_point(data = combined_gwas_result_heatmap, aes(colour = P.value), shape = 108, size = 5.5, na.rm = TRUE) +
                scale_colour_gradientn(colours=c("red","orange", "green","blue")) +
                theme(
            ...
            ggsave(filename = "GAPIT.combined.GWAS.Results.heatmap.png", plot = p, path = file.path(output_path))
        }
    }, error = function(e) {
